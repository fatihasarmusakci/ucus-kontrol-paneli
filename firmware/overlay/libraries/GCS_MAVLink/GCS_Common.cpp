...
#include <AP_EnergyRecovery/AP_EnergyRecovery.h>
...
ap_message GCS_MAVLINK::mavlink_id_to_ap_message_id(const uint32_t mavlink_id) const
{
    ...
    static const struct {
        ...
        { MAVLINK_MSG_ID_ENERGY_RECOVERY_DATA,  MSG_ENERGY_RECOVERY_DATA},
#endif
...
#if AP_ENERGY_RECOVERY_ENABLED
void GCS_MAVLINK::send_energy_recovery_data() const
{
    AP_EnergyRecovery *er = AP_EnergyRecovery::get_singleton();
    if (er == nullptr) {
        return;
    }
    er->send_energy_recovery_data(*this);
}
...
#if AP_ENERGY_RECOVERY_ENABLED
    case MSG_ENERGY_RECOVERY_DATA:
        CHECK_PAYLOAD_SIZE(ENERGY_RECOVERY_DATA);
        send_energy_recovery_data();
        break;
#endif
